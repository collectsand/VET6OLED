...
{
    uint8_t dat[2];
    HAL_GPIO_WritePin(SPI1_CS_GPIO, SPI1_CS_PIN, GPIO_PIN_RESET);

    dat[0] = cmd | ICM20602_SPI_W;
    dat[1] = val;
    HAL_SPI_Transmit(&ICM_SPI_HANDLE, dat, 2, 500);

    HAL_GPIO_WritePin(SPI1_CS_GPIO, SPI1_CS_PIN, GPIO_PIN_SET);
}
...
void icm_spi_r_reg_byte(uint8_t cmd, uint8_t *val)
{
    ...
    dat[1] = *val;
    HAL_SPI_TransmitReceive(&ICM_SPI_HANDLE, dat, dat, 2, HAL_MAX_DELAY);
...
{
    HAL_GPIO_WritePin(SPI1_CS_GPIO, SPI1_CS_PIN, GPIO_PIN_RESET);
    HAL_SPI_TransmitReceive(&ICM_SPI_HANDLE, val, val, num, HAL_MAX_DELAY);
    HAL_GPIO_WritePin(SPI1_CS_GPIO, SPI1_CS_PIN, GPIO_PIN_SET);
}
